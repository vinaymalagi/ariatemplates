...
import { isEmpty, remove, contains } from '../utils/Array.js';
...
import { ViewportNavigationInterceptor } from '../utils/DomNavigationManager.js';
...
export const PoupManager = classDefinition({
  ...
  $prototype: {
    ...
    connectEvents: function () {
      ...
      ariaUtilsEvent.addListener(this._document, "mousedown", {
        fn: this.onDocumentClick,
        scope: this
      }, true);
      ariaUtilsEvent.addListener(this._document, "touchend", {
        fn: this.onDocumentClick,
        scope: this
      }, true);

      ariaUtilsEvent.addListener(this._document, "mouseout", {
        fn: this.onDocumentMouseOut,
        scope: this
      }, true);
      ariaUtilsEvent.addListener(this._document, "mousewheel", {
        fn: this.onDocumentMouseScroll,
        scope: this
      }, true);
      ariaUtilsEvent.addListener(this._document.body, "focusin", {
      ...
      if (ariaCoreBrowser.isOldIE) {
        ...
        ariaUtilsEvent.addListener(FRAMEWORK_GLOBALS.$window, "mousewheel", {
          fn: this._onScroll,
          scope: this
        }, true);
    ...
    disconnectEvents: function () {
      if (this._document) {
        ariaUtilsEvent.removeListener(this._document, "mousedown", {
          fn: this.onDocumentClick
        });
        ariaUtilsEvent.removeListener(this._document, "touchend", {
          fn: this.onDocumentClick
        });
        ariaUtilsEvent.removeListener(this._document, "mouseout", {
          fn: this.onDocumentMouseOut
        });
        ariaUtilsEvent.removeListener(this._document, "mousewheel", {
          fn: this.onDocumentMouseScroll
        });
        ariaUtilsEvent.removeListener(this._document.body, "focusin", {
          fn: this.onDocumentFocusIn
        });
        ariaUtilsAriaWindow.detachWindow();
        this._document = null;
        if (ariaCoreBrowser.isOldIE) {
          ariaUtilsEvent.removeListener(FRAMEWORK_GLOBALS.$window, "mousewheel", {
            fn: this._onScroll
          });
        } else {
    ...
    findParentPopup: function (target, notifyTargetBehindModalPopup) {
      ...
      if (searchPopup) {
        for (var i = this.openedPopups.length - 1; i >= 0; i--) {
          var popup = this.openedPopups[i];
          if (ariaUtilsDom.isAncestor(target, popup.getDomElement())) {
            // the element is in the modal popup, it is fine to focus it
            return popup;
          }
          if (popup.modalMaskDomElement && ariaUtilsDom.isAncestor(target, popup.popupContainer.getContainerElt())) {
    ...
    bringToFront: function (popup) {
      ...
      for (i = 0, l = popupsToKeepInFront.length; i < l; i++) {
        const curPopup = popupsToKeepInFront[i];
    ...
    onDocumentClick: function (event) {
      ...
      for (var j = 0, length = popup._ignoreClicksOn.length; j < length; j++) {
        if (ariaUtilsDom.isAncestor(target, popup._ignoreClicksOn[j])) {
          ignoreClick = true;
          break;
        }
      }

      if (!(ignoreClick || ariaUtilsDom.isAncestor(target, popup.getDomElement()))) {
    ...
    onDocumentMouseOut: function (event) {
      ...
      for (var i = this.openedPopups.length - 1; i >= 0; i--) {
        var popup = /** @type aria.popups.Popup */
          this.openedPopups[i];
        if (!ariaUtilsDom.isAncestor(toTarget, popup.getDomElement())
          && ariaUtilsDom.isAncestor(fromTarget, popup.getDomElement())) {
      ...
      for (var i = this.openedPopups.length - 1; i >= 0; i--) {
        ...
        if (!ariaUtilsDom.isAncestor(target, popup.getDomElement())) {
          ...
          if (ariaCoreBrowser.isFirefox) {
            ...
            if (ariaUtilsDom.isAncestor(this._document.elementFromPoint(domEvent.clientX, domEvent.clientY), popup.getDomElement()))
              continue;
          }
    ...
    show: function (popupConf) {
      // TODO: ModernAria: CircularDependency: between PopupManager and Popup, resolved with getClassRef
      const popupConstructor = getClassRef('aria.popups.Popup');
      var popup = new popupConstructor();
    ...
    unregisterPopup: function (popup) {
      if (contains(this.openedPopups, popup)) {
        remove(this.openedPopups, popup);
      }
      if (contains(this.popups, popup)) {
        remove(this.popups, popup);
      }
      if (isEmpty(this.openedPopups)) {
        this.currentZIndex = this.baseZIndex;
      }
