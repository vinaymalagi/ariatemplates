...
import { z } from 'zod';
import { PackageNameSchema } from './json-types-zod.js';
...
// Original Bean definitions for Interface member configs
...
/**
 * Enum of valid values for $type property in interface member type definition
 */
export const ItfMemberTypesEnum = z.union([
  z.string('Function'),
  z.string('Object'),
  z.string('Interface')
]);
...
/**
 * Definition for interface methods. Methods can either be synchronous, or asynchronous with a callback parameter.
 *
 ...
 * </pre>
 */
export const ItfMemberFunctionCfgSchema = z.object({
  $type: z.literal('Function'),
  /**
   * Must be null (or undefined) if the method is synchronous. If the method is asynchronous, must contain the index of the parameter which contains the callback (0 = first parameter).
   */
  $callbackParam: z.number().int().optional().nullish()
}).passthrough();

/**
 * Definition for interface objects. Objects are not checked and simply copied in the interface wrapper.
 * <pre>
 */
export const ItfMemberObjectCfgSchema = z.object({
  $type: z.literal('Object')
}).passthrough();

/**
 * Definition for members of interface which are other interfaces. When creating the interface wrapper, the element generated by such a member will always be an interface wrapper with the classpath specified in the $classpath property, whether it was an interface wrapper or a whole object in the whole object.
 */
export const ItfMemberInterfaceCfgSchema = z.object({
  $type: z.literal('Interface'),
  /**
   * Classpath of the interface which describes the interface member.
   */
  $classpath: PackageNameSchema
});

export const ItfMemberCfgSchema = z.discriminatedUnion('$type', [
  ItfMemberFunctionCfgSchema,
  ItfMemberInterfaceCfgSchema,
  ItfMemberObjectCfgSchema
]);
