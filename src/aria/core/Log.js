...
import { DefaultAppender } from './log/DefaultAppender.js';
...
class Log {
  ...
  constructor() {
    ...
    // MUST_DO: ModernAria: Figure out setting of default logging level. For now using debug.
    // this.setLoggingLevel("*", Aria.debug ? this.LEVEL_DEBUG : this.LEVEL_ERROR);
    this.setLoggingLevel("*", LEVEL_DEBUG);
    // MUST_DO: ModernAria: Figure out default  appender instantiation
    this.addAppender(new DefaultAppender());
  }
   ...
   */
  isValidLevel(level) {
    return (level === LEVEL_DEBUG || level === LEVEL_INFO || level === LEVEL_WARN || level === LEVEL_ERROR);
  }
   ...
   */
  setLoggingLevel(className, level) {
   ...
   */
  prepareLoggedMessage(msg, msgArgs, errorContext) {
    if (msgArgs) {
      msg = substitute(msg, msgArgs);
    }
    for (var error in errorContext) {
      if (Object.prototype.hasOwnProperty.call(errorContext, error)) {
 ...
 */
  debug(className, msg, msgArgs, o) {
    this.log(className, LEVEL_DEBUG, msgArgs, msg, o);
  }
   ...
   * @param {Object} o An optional object to inspect
   */
  info(className, msg, msgArgs, o) {
    this.log(className, LEVEL_INFO, msgArgs, msg, o);
  }
   ...
   * @param {Object} o An optional object to inspect
   */
  warn(className, msg, msgArgs, o) {
    this.log(className, LEVEL_WARN, msgArgs, msg, o);
  }
   ...
   */
  error(className, msg, msgArgs, e) {
    this.log(className, LEVEL_ERROR, msgArgs, msg, e);
  }
   ...
   * levels
   */
  log(className, level, msgArgs, msgText, objOrErr) {
    if (!this.isValidLevel(level)) {
