 ...
 */
export const AriaDomEvent = classDefinition({
  ...
  $constructor: function (evt) {
    ...
    if (!domEventCopy && isIE8orLess && ariaWindow.event) {

      evt = ariaWindow.event;
      // create all object properties from the map
      for (const k in eventMap) {
        if (Object.prototype.hasOwnProperty.call(eventMap, k)) {
