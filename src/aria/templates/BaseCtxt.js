...
import { getClassInstance, getClassRef } from '../core/class-registry.js';
 ...
 */
export const BaseCtxt = classDefinition({
  ...
  $prototype: {
    ...
    __loadLibs: function (libsMap, libsType) {
      ...
      let libsArray, ctxtLibsContainerName, libCtxtClass, itf;
      if (libsType == "macrolibs") {
        libsArray = this._macrolibs;
        ctxtLibsContainerName = "__$macrolibs";
        libCtxtClass = getClassRef('aria.templates.TemplateCtxt');
        itf = getClassRef('aria.templates.ITemplate');
      } else if (libsType == "csslibs") {
        libsArray = this._csslibs;
        ctxtLibsContainerName = "__$csslibs";
        libCtxtClass = getClassRef('aria.templates.CSSCtxt');
        itf = getClassRef('aria.templates.ICSS');
      }
      ...
      for (const handle in libsMap) {
        if (Object.prototype.hasOwnProperty.call(libsMap, handle)) {
      ...
      for (const handle in libsMap) {
        if (Object.prototype.hasOwnProperty.call(libsMap, handle)) {
          ...
          this._tpl[handle] = getClassInstance(libsMap[handle]);
    ...
    __disposeLibs: function (libs) {
      for (var i = 0; i < libs.length; i++) {
        var lib = libs[i];
        const iTemplate = getClassRef('aria.templates.ITemplate');
        if (iTemplate) {
          iTemplate.prototype.$destructor.call(lib._tpl);
        }
        lib.$dispose();
      }
    }
