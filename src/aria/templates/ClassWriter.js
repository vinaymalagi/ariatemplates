 ...
 */
export const ClassWriter = classDefinition({
    ...
    $constructor : function (processStatement, processErrors) {
         ...
         */
        this.sourceFileDir = null;

        /**
         * Path to the directory with all sources.
         */
        this.sourcesRootDirPath = null;

        /**
         * Whether we are generating classes for internal Ariatemplates tmplates
         */
        this.isInternalAriatemplatesBuild = false;
    ...
    $prototype : {
        ...
        addDependency : function (dependency, extension) {
          const modulePath = this.isInternalAriatemplatesBuild ? convertInternalImportToRelativePath(dependency.modulePath, this.sourceFilePath, this.sourcesRootDirectory) : dependency.modulePath;

          let moduleItem = this._dependencies.get(modulePath);
          if(!moduleItem) {
            moduleItem = {
              namedImports: new Map(),
              importAsSideEffect: false,
          ...
          if (dependency.importType === "sideEffect") {
            ...
            let namedImportsItem = moduleItem.namedImports.get(name);
            if (!namedImportsItem) {
              namedImportsItem = {hasWithoutAliasImport: false, aliases: new Set()};

              moduleItem.namedImports.set(name, namedImportsItem);
            }
            if(!dependency.alias) {
              namedImportsItem.hasWithoutAliasImport = true;
            } else {
              namedImportsItem.aliases.add(dependency.alias);
            }
          } else if (dependency.importType === "Namespace") {
            moduleItem.namespaceImports.add(dependency.name);
          } else if (dependency.importType === "Default") {
            moduleItem.defaultImports.add(dependency.name);
          }
        },
        ...
        writeDependencies : function () {
            ...
            //     if (Object.prototype.hasOwnProperty.call(this._dependencies, i)) {
            //         this.writeln("require(", this.stringify(i), ");");
            //     }
            // }
            for (const [modulePath, moduleItem] of this._dependencies) {
              // TODO: ModernAria: Add warnings for mixed/multiple import types or convert templates specifications to use classes.

              // Add side effect import line
              if(moduleItem.importAsSideEffect) {
                this.writeln("import '", modulePath, "';");
              }

              // Add default import lines (ideally there should be only one default import per module) MUST_DO: ModernAria: Add warning for multiple default imports
              moduleItem.defaultImports.forEach((defaultImport) => {
                this.writeln("import ", defaultImport, " from '", modulePath, "';");
              });

              // Add namespace import lines (ideally there should be only one default import per module) MUST_DO: ModernAria: Add warning for multiple namespace imports
              moduleItem.namespaceImports.forEach((namespaceImport) => {
                this.writeln("import * as ", namespaceImport, " from '", modulePath, "';");
              });

              // Add named import lines
              if(moduleItem.namedImports.size > 0) {
                const importBuilder = [];
                for (const [name, namedImportsItem] of moduleItem.namedImports) {

                  if (namedImportsItem.hasWithoutAliasImport) {
                    importBuilder.push(name);
                  }
                  namedImportsItem.aliases.forEach((alias) => {
                    importBuilder.push(`${name} as ${alias}`);
                  });
                }
                if(importBuilder.length > 0) {
                  this.writeln("import { ", importBuilder.join(", "), " } from '", modulePath, "';");
                }
              }

            }

            this.writeln("//File and dirname constants");
            this.writeln(`const __filename=${this.stringify(basename(this.sourceFilePath))};`);
            this.writeln(`const __dirname=${this.stringify(dirname(this.sourceFilePath))};`);
