...
import { isDate, isString } from '../utils/Type.js';
import { escapeForHTML, stripAccents } from '../utils/String.js';
...
var regExSpecials, __modifiers = {
  ...
  "dateformat": {
    ...
    fn: function (date, pattern) {
      if (isDate(date)) {
        return getClassRef('aria.utils.Date').format(date, pattern);
      } else {
        this.$logError(Modifiers.DATEFORMAT_MODIFIER_ENTRY, [date]);
      }
    }
  },
  "timeformat": {
    ...
    fn: function (time, pattern) {
      if (isDate(time)) {
        return getClassRef('aria.utils.Date').format(time, pattern);
      } else {
        this.$logError(Modifiers.DATEFORMAT_MODIFIER_ENTRY, [time]);
      }
    }
  ...
  "highlightfromnewword": {
    ...
    fn: function (str, highlight) {
      ...
      if (isString(str) && highlightLen) {
        ...
        var middle = stripAccents(middleOriginal).toLowerCase();
        if (middle === highlight) {
          return escapeForHTML(str.substring(0, a)) + "<strong>" + escapeForHTML(middleOriginal) + "</strong>" + escapeForHTML(str.substring(b));
        }
      }
      return str ? escapeForHTML(str) : str;
    }
  },
  "starthighlight": {
    /**
     ...
     */
    fn: function (str, highlight) {
      if (isString(str) && highlight.length) {
        var beginning = stripAccents(str.substring(0, highlight.length)).toLowerCase();
        highlight = highlight.toLowerCase();
        if (beginning === highlight) {
          return "<strong>" + escapeForHTML(str.substring(0, highlight.length)) + "</strong>"
            + escapeForHTML(str.substring(highlight.length));
        }
      }
      return str ? escapeForHTML(str) : str;
    }
  ...
  "highlight": {
     ...
     */
    fn: function (str, highlight) {
      if (isString(str) && isString(highlight)) {
        ...
        forEach(toBeMatched, function (value, index, array) {
  ...
  $prototype: {
    ...
    callModifier: function (modifierName, params) {
      ...
      if (modifier) {
        ...
        this.$logError(Modifiers.UNKNOWN_MODIFIER, [modifierName]);
      }
    },
