...
import { emptyFn } from '../common/fixed-return-value-functions.js';
    ...
    var errorInTemplateDiv = function (args) {
        ...
        if (div) {
            getClassRef('aria.utils.Dom').replaceHTML(div, "#TEMPLATE ERROR#");
        }
    ...
    var __loadTemplate3 = function (res, args) {
        ...
        var cfg = args.cfg;
        const templateCtxtCnstr = getClassRef('aria.templates.TemplateCtxt');
        var tplCtxt = new templateCtxtCnstr();
    ...
    var __loadTemplate1 = function (args) {
        ...
        var classes = [import('./TemplateCtxt.js'), import('./CSSMgr.js')]; // classes to load in addition to the
        // template
        var moduleCtrl = cfg.moduleCtrl;
        if (moduleCtrl && !moduleCtrl.getData) {
        ...
        if (cfg.reload) {
            getClassRef('aria.templates.TemplateManager').unloadTemplate(cfg.classpath, cfg.reloadByPassCache);
            const cssMgr = getClassRef('aria.templates.CSSMgr');
            if (cssMgr) {
                // MUST_CHECK: ModernAria: Is this still needed?
                // cssToReload = cssToReload.concat(cssMgr.getInvalidClasspaths(true));
            }
        }

        // Set the correct size for the div:
        var layout = getClassRef('aria.templates.Layout');
        if (cfg.rootDim) {
            layout.setRootDim(cfg.rootDim);
        }
        var div = cfg.div;
        div = getClassRef('aria.utils.Dom').replaceHTML(div, "");
        if (!div) {
            // error should have already been reported
            // eslint-disable-next-line no-invalid-this
            this.$callback(cb, {
        ...
        var tplDiv = FRAMEWORK_GLOBALS.$window.document.createElement('div');
     ...
     */
    export const TplClassLoader = classDefinition({
        ...
        $statics : {
            ...
            _importScriptPrototype : function (scriptClass, tplPrototype) {
                ...
                if (tplPrototype[refScriptProto]) {
                    return this.$logError(DUPLICATE_CLASSNAME, [scriptDef.$classpath]);
                }
                    ...
                    for (const member in scriptResources) {
                        if (Object.prototype.hasOwnProperty.call(scriptResources, member)) {
                            if (tplPrototype[member] && !tplPrototypeRes[member]) {
                                this.$logError(RESOURCES_HANDLE_CONFLICT, [member, scriptDef.$classpath]);
                            } else {
                    ...
                    for (const member in scriptTexts) {
                        if (Object.prototype.hasOwnProperty.call(scriptTexts, member)) {
                            if (tplPrototype[member] && !tplPrototypeText[member]) {
                                this.$logError(TEXT_TEMPLATE_HANDLE_CONFLICT, [member, scriptDef.$classpath]);
                            } else {
            ...
            _startLoad : function (evt, args) {
                ...
                Promise.all([
                  ...
                  import('./CfgBeans.js'),
                  import('../utils/Dom.js'),
                  import('./TemplateManager.js')
                ]).then(() => {
                  __loadTemplate1.call(this, {cfg, cb});
                });
            ...
            disposeTemplate : function (div) {
                const ariaUtilsDom = getClassRef('aria.utils.Dom');
                if (typeof(div) == "string") {
                    div = ariaUtilsDom?.getElementById(div);
                }
                if (ariaUtilsDom) {
                    // MUST_CHECK: ModernAria: What if The template context manager is not loaded?
                    return getClassRef('aria.templates.TemplateCtxtManager').disposeFromDom(div);
                }
            }
        }
    });
