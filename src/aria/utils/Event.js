...
var EL = 0;
var TYPE = 1;
// MUST_DO: ModernAria: Confirm lastError not used
// var lastError = null;
  ...
  $prototype: {
    ...
    reset: function () {
      if (unloadListeners) {
        ...
        this._simpleRemove(FRAMEWORK_GLOBALS.$window, "unload", this._unload);
      }
    ...
    _checkUnloadListeners: function () {
      if (unloadListeners == null) {
        unloadListeners = [];
        this._simpleAdd(FRAMEWORK_GLOBALS.$window, "unload", this._unload);
      }
    },
     ...
     */
    _simpleAdd: function () {
      const ariaWindow = FRAMEWORK_GLOBALS.$window;
      if (ariaWindow) {
        if (ariaWindow.addEventListener) {
          return function (element, event, callback, capture) {
            element.addEventListener(event, callback, (capture));
          };
        } else if (ariaWindow.attachEvent) {
    ...
    _addListener: function (element, event, callback, scope, args, useCapture) {
      ...
      if (event != "mousemove" || !ariaCoreBrowser.isWebkit) {
        wrappedCallback = function (e) {
          const ariaTouchClickBuster = getClassRef('aria.touch.ClickBuster');
          ...
          if (
            (e.type != "click" && e.type != "mousedown" && e.type != "touchend")
            || !ariaTouchClickBuster || ariaTouchClickBuster.preventGhostClick(e)) {
            return handlerCBInstance.call(UtilsEvent.getEvent(e, element));
          }
        };
      } else {
        ...
        wrappedCallback = function (e) {
          ...
          e = UtilsEvent.getEvent(e, element);
    ...
    getEvent: function (e, boundElement) {
      const ariaWindow = FRAMEWORK_GLOBALS.$window;
      var ev = e || ariaWindow.event;

      if (!ev) {
        var Event = ariaWindow.Event;
    ...
    _simpleRemove: function () {
      const ariaWindow = FRAMEWORK_GLOBALS.$window;
      if (ariaWindow) {
        if (ariaWindow.removeEventListener) {
          return function (element, event, callback, capture) {
            element.removeEventListener(event, callback, (capture));
          };
        } else if (ariaWindow.detachEvent) {
