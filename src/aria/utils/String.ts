 ...
 */
export function isEscaped(str: string, index: number) {
    let res = false;
 ...
 */
export function indexOfNotEscaped(str: string, findChar: string, start: number = 0) {
    let index = str.indexOf(findChar, start);
    while (index != -1) {
 ...
 */
export function nextWhiteSpace (str: string, start: number, end: number, regexp: RegExp = /\s/) {
    let currentCharPos = start;
    const whiteRegexp = regexp;
    let currentChar;
 ...
 * @return {String}
 */
export function stringify (param: string) {
