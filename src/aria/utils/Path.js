 ...
 * TODO: ModernAria: JsDoc: Add module Jsdoc comment
 */
import { FRAMEWORK_GLOBALS } from '../core/framework-bootstrap.js';
import { indexOfNotEscaped } from './String.js';
import { isArray, isObject, isNumber, isString } from './Type.js';



const WRONG_PATH_SYNTAX = "Syntax for path %1 is not valid. Note that variables cannot be used in paths.";
const RESOLVE_FAIL = "Resolve for path %1 failed, root element does not contain this path";
 ...
 */
export function setValue(inside, path, value) {
  if (!inside || !path || !isObject(inside)) {
    return;
  }
  if (isString(path)) {
    path = parse(path);
  }
  if (!isArray(path) || isNumber(path[0])) {
    return;
  }
  ...
  for (var i = 0, len = path.length; i < len; i += 1) {
    ...
    if (i === len - 1) {
      ...
      next = path[i + 1];
      if (isNumber(next) && !isArray(container[param])) {
        container[param] = [];
      }
      if (isString(next) && (!isObject(container[param]) || !(param in container))) {
        container[param] = {};
      }
 ...
 */
export function resolve(path, inside) {
  if (isString(path)) {
    path = parse(path);
  }
  inside = inside ? inside : FRAMEWORK_GLOBALS.$window;
  if (isArray(path)) {
...
function _paramParse(path) {
  ...
  if (first == "[") {
    ...
    if (closing != -1) {
      ...
      if (/^\d+$/.test(part)) {
        nextParse = _paramParse(next);
        nextParse.unshift(parseInt(part, 10));
        return nextParse;
      } else {
        // check that part is "something" or 'somethingelse'
        var strMarker = part.charAt(0);
        if ((strMarker == "'" || strMarker == '"')
          && indexOfNotEscaped(part, strMarker, 1) == part.length - 1) {
          nextParse = _paramParse(next);
 ...
 * @return {String}
 */
export function pathArrayToString(pathParts) {
 ...
 * @return {Object}
 */
export function describe(container, path) {
  if (!container || !path || !isObject(container)) {
    return;
  }
  if (isString(path)) {
    path = parse(path);
  ...
  for (var i = 0, len = path.length; i < len; i += 1) {
    ...
    if (isNumber(param) && !isArray(obj)) {
      return null;
    }
    if (isString(param) && (!isObject(obj) || !(param in obj))) {
      return null;
    }
