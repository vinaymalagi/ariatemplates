...
import { JsonSerializer } from './json/JsonSerializer.js';
import { keys } from './Object.js';
import { isArray, isDate, isNumber, isObject, isString } from './Type.js';
...
var __getListenerMetaName = function (property, recursive) {
  var metaName = recursive ? Json.META_FOR_RECLISTENERS : Json.META_FOR_LISTENERS;
...
var __removeLinkToParent = function (oldChild, parent, property) {
  ...
  if (oldChildParents) {
    // TODO: ModernAria: Check for refactor, maybe check against length
    for (let index = 0; oldChildParents[index]; index++) {
      const parentDescription = oldChildParents[index];
...
var __changeLinkToParent = function (child, parent, oldProperty, newProperty) {
  ...
  if (childParents) {
    // TODO: ModernAria: Check for refactor, maybe check against length
    for (let index = 0; childParents[index]; index++) {
      const parentDescription = childParents[index];
...
var __includePropForBackRef = function (propertyName) {
  ...
  // TODO: ModernAria: change use of substr to substring as substr is deprecated.
  // which property names are included for back references
  return (propertyName != Json.OBJECT_PARENT_PROPERTY &&
    propertyName.substr(0, Json.META_FOR_LISTENERS.length) != Json.META_FOR_LISTENERS &&
    propertyName.substr(0, Json.META_FOR_RECLISTENERS.length) != Json.META_FOR_RECLISTENERS);
};
...
var __checkAndRemoveBackRefs = function (container) {
  ...
  var recListenersPrefix = Json.META_FOR_RECLISTENERS;
  var recListenersPrefixLength = recListenersPrefix.length;
  for (var childName in container) {
    if (Object.prototype.hasOwnProperty.call(container, childName) && childName.substr(0, recListenersPrefixLength) === recListenersPrefix) {
  ...
  for (const childName in container) {
    if (Object.prototype.hasOwnProperty.call(container, childName) && __includePropForBackRef(childName)) {
...
var __cleanUpRecMarkers = function (node) {
  var recMarker = Json.TEMP_REC_MARKER;
  if (node[recMarker]) {
    node[recMarker] = false;
    delete node[recMarker];
    var parents = node[Json.OBJECT_PARENT_PROPERTY];
    if (parents) {
...
var __retrieveListeners = function (node, property, recursive, listeners) {
  ...
  if (parents) {
    ...
    node[Json.TEMP_REC_MARKER] = true;

    // TODO: ModernAria: Check for refactor, maybe check against length
    for (let index = 0, parentDesc; parents[index]; index++) {
      parentDesc = parents[index];
 ...
 * @singleton
 */
export const Json = classDefinition({
  ...
  $constructor: function () {
     ...
     */
    this.__defaultJsonSerializer = new JsonSerializer(true);
  },
  $destructor: function () {
    parProp = null;
  ...
  $statics: {
     ...
     * @type String
     */
    META_FOR_LISTENERS: FRAMEWORK_PREFIX + 'listener',

    /**
     * Meta used in json to store recursive listeners
     * @type String
     */
    META_FOR_RECLISTENERS: FRAMEWORK_PREFIX + 'reclistener',

    /**
     * Property name for the parent information container
     * @type String
     */
    OBJECT_PARENT_PROPERTY: FRAMEWORK_PREFIX + 'parent',

    /**
     * Property name for recursive temporary markers
     * @type String
     */
    TEMP_REC_MARKER: FRAMEWORK_PREFIX + 'tempRecMarker',
  ...
  $prototype: {
    ...
    splice: function (container, index, howManyRemoved) {
      ...
      if (!(isArray(container) && isNumber(index) && isNumber(howManyRemoved)
    ...
    setValue: function (container, property, val, listenerToExclude, throwError) {
      ...
      var exists = Object.prototype.hasOwnProperty.call(container, property);
      var oldVal = container[property];
    ...
    addListener: function (container, property, callback, throwError, recursive) {
      ...
      if (!container[meta]) {
        ...
        if (indexOf(listeners, callback) > -1) {
    ...
    removeListener: function (container, property, callback, recursive) {
      ...
      var listeners = container[meta], idx = indexOf(listeners, callback);
      if (idx > -1) {
    ...
    load: function (str, ctxt, errMsg) {
      ...
      if (typeof JSON !== "undefined" && typeof JSON.parse === "function") {
        try {
          ...
          // TODO: ModernAria: Is this fallback to eval necessary??
    ...
    inject: function (src, target, merge) {
      ...
      var allArray = (isArray(src) && isArray(target));
      var allObject = (isObject(src) && isObject(target));
      var elem;
    ...
    contains: function (big, small) {
      var isBigArray = isArray(big), isBigObject = isObject(big), isSmallArray = isArray(small), isSmallObject = isObject(small);
    ...
    keys: function (container) {
      var raw = keys(container);
      // NOTE: ModernAria: Benefit: Arrow functions to retain this scope.
      return raw.filter((key) => this.isMetadata(key));
    },
    ...
    equalsDiff: function (left, right) {
      var isLeftArray = isArray(left),
        isLeftObject = isObject(left),
        isRightArray = isArray(right),
        isRightObject = isObject(right),
        leftDiff = null,
        rightDiff = null,
        // TODO: ModernAria: refactor: maybe use arrow function and will not have to use .call synctax to call the function
        __processContainer = function (list, __getKey) {
          ...
          for (var i = 0, l = list.length; i < l; i++) {
            ...
            // TODO: ModernAria: Ripe for refactor, maybe use arrow function and will not have to use .call synctax to call the function
      ...
      } else if (isDate(right) && isDate(left)) {
